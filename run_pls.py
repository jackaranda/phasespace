...
#	values = np.ma.masked_less(predictand_data[:,id], 0.1)
	values = predictand_data[:,id]
	print values
	ax.scatter(np.array(tree.samples)[:,0], np.array(tree.samples)[:,1], c='grey', s=5, alpha=0.3)
	ax.scatter(np.array(tree.samples)[:,0], np.array(tree.samples)[:,1], c=values, s=30, alpha=0.8, cmap='RdBu_r', vmin=np.min(values), vmax=np.max(values))
